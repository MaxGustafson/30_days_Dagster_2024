from dagster import asset, asset_check, AssetExecutionContext, AssetCheckResult, AssetCheckSeverity, Config, ConfigurableResource, Definitions, file_relative_path, MetadataValue, MaterializeResult, PipesSubprocessClient,  RetryPolicy, RunRequest, sensor, SkipReason,  SensorEvaluationContext 
import pandas as pd
import duckdb
import os, time
...
def cashflow_raw(context: AssetExecutionContext, pipes_subprocess_client : PipesSubprocessClient):
    """ Reads in Raw data from Cashflow_Interest.""" 

    cmd = [shutil.which("python"), file_relative_path(__file__, "external_python_code.py")]
...
def check_data_exists_cashflow_raw(csv_resource_cleansed : my_csv_handler): 
    df : pd.DataFrame = csv_resource_cleansed.read_csv()
...
defs = Definitions(
    ...
    asset_checks= [check_data_exists_cashflow_raw],
